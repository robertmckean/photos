#!/usr/bin/env python3
"""
v4_batch_next_1000.py

Processes the next 1000 NOT_STARTED rows from v4_safe_progress.csv:
...
REPORT_CSV = Path(r"C:\Users\windo\VS_Code\photos\results\v4_batch_next_1000_report.csv")
...
def main() -> None:
    ...
    with open(PROGRESS_CSV, encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            if row.get("progress_status") == "NOT_STARTED":
                not_started.append(row)

    batch = not_started[:BATCH_SIZE]
    print(f"  NOT_STARTED total: {len(not_started):,}")
